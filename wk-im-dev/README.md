...
日常有三种启动方式，按你的工作上下文选择。

---

### 场景 A：在 BTIMService 或 BTIMModule 仓库中独立开发

适用于：只需要改一个组件，不需要跨仓库联调。
...
# BTIMModule 同理：
# bash wk-im-dev/scripts/install.sh --runtime codex --target /path/to/BTIMModule

# 第 2 步：初始化知识库（只需一次，后续自动维护）
~/.wk-im-dev/bin/wk-im-init.sh --root /path/to/BTIMService

# 第 3 步：开始工作
cd /path/to/BTIMService
codex
# 或 Claude Code：claude --plugin-dir /path/to/Agents/wk-im-dev
```

---

### 场景 B：在 HostApp 中通过 CocoaPods 本地依赖开发两个组件

适用于：从主 App 同时开发 BTIMService 和 BTIMModule，Podfile 已配置本地 `:path =>`。
...
# 第 1 步：安装（只需一次）
cd /path/to/Agents
bash wk-im-dev/scripts/install.sh --runtime codex --target /path/to/HostApp

# 第 2 步：初始化（从 HostApp 根目录运行，会自动解析两个组件路径并初始化各自知识库）
~/.wk-im-dev/bin/wk-im-init.sh --root /path/to/HostApp
# init 成功后会生成：
#   /path/to/HostApp/.wk-im-workspace.json     （项目级配置）
#   ~/.wk-im-dev/workspace.json                （全局配置，guard/verify 的跨仓库 fallback）

# 第 3 步：在 HostApp 目录开始工作
cd /path/to/HostApp
codex
```

> **关于本地 pod 不需要 pod install**：使用 `:path =>` 本地依赖时，修改组件源文件后**直接 build** 即可，Xcode 通过路径实时读取最新源码。`wk-im-verify.sh` 会自动从 HostApp xcworkspace build，无需手动干预。

> **跨仓库提交顺序**：先 commit BTIMService（含 public header + knowledge contracts），再 commit BTIMModule（调用方变更），两个仓库各自保持可独立 review 的 git 历史。

---

### 场景 C：已在组件仓库，需要临时访问另一个组件路径

适用于：Claude Code 打开在 BTIMService，但某次改动涉及 BTIMModule（或反之）。

```bash
# 在 HostApp 完成一次初始化后，全局配置会记录两个组件路径：
~/.wk-im-dev/bin/wk-im-init.sh --root /path/to/HostApp

# 之后即使 Claude Code 打开在 BTIMService 仓库，
# guard/verify 脚本也能通过 ~/.wk-im-dev/workspace.json 找到 BTIMModule 并做跨组件检查。
# 如果自动检测失败，手动指定：
...
| 场景 | 做法 |
| --- | --- |
| 只在一个组件仓库内工作 | `install.sh --runtime codex --target <BTIMService 或 BTIMModule>` |
| HostApp 同时改两个组件 | `install.sh --target <HostApp>` + `wk-im-init.sh --root <HostApp>` |
| 路径自动识别失败 | `wk-im-init.sh --service <p> --module <p> --host-app <p>` 手动指定 |
| 已有 `AGENTS.md` | installer 保留原内容，合并/更新 `<!-- WK-IM-DEV:START/END -->` 区块 |
| 只安装命令脚本，不写项目入口 | `install.sh --target <repo> --skip-project-agents` |
| 不更改 shell rc | `install.sh --target <repo> --no-shell-rc` |
| 整体替换项目入口 | `install.sh --target <repo> --replace-project-agents`（自动备份原文件） |
| 只用 Claude Code plugin | `install.sh --runtime claude --target <repo>`（不写 CLAUDE.md） |
