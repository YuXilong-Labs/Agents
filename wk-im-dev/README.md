...
> **从 v3.2 升级到 v3.3**：marketplace name 从 `yuxilong-labs` 改为 `yuxilong-agents`。如果你之前装过旧名字，先 remove 再重新 add：
>
> ```bash
...
**slash command 等效形式（在 claude 交互界面内）：** `/plugin install wk-im-dev@yuxilong-agents`、`/plugin uninstall wk-im-dev@yuxilong-agents`。终端命令与 slash command 任选其一。
...
`wk-im-init.sh` 会自动检测并安装 [CodeGraph](https://github.com/colbymchenry/codegraph)，给组件仓库建立 AST 知识图谱，让 `wk-im-explorer` 和 `wk-im-debugger` 用 `codegraph_*` MCP 工具加速结构化查询：

- **~35% 更便宜、~70% 更少工具调用、100% 本地**（基准来自 codegraph 上游）
...
| Claude Code 全局安装 | `claude plugin install wk-im-dev@yuxilong-agents`（需先注册 marketplace） |
...
| CodeGraph 检测 | `wk-im-codegraph.sh detect` |
| CodeGraph 安装 | `wk-im-codegraph.sh install [--yes]` |
| CodeGraph 建索引 | `wk-im-codegraph.sh init --root <repo>` |
| CodeGraph 索引状态 | `wk-im-codegraph.sh status --root <repo>` |
| **卸载** | `bash scripts/uninstall.sh [--target <repo>]` 或 `curl -fsSL …/uninstall.sh \| bash -s -- [--target <repo>]` |
...
```
/plugin install wk-im-dev@yuxilong-agents
```
...
├── skills/
│   └── im-knowledge/
│       ├── constraints-core.md       # 硬约束清单(subagent inline)
│       ├── constraints-extended.md   # 详细 rationale(主 agent/skills)
│       └── constraints.md            # 兼容入口
└── examples/
```
...
bash scripts/uninstall.sh --target <repo>    # 同时清理目标仓库的 AGENTS.md marker 区块

# 无本地仓库时（curl 方式）
curl -fsSL https://raw.githubusercontent.com/YuXilong-Labs/Agents/main/wk-im-dev/scripts/uninstall.sh \
  | bash
# 同时清理目标仓库：
curl -fsSL https://raw.githubusercontent.com/YuXilong-Labs/Agents/main/wk-im-dev/scripts/uninstall.sh \
  | bash -s -- --target /path/to/repo

# Claude Code plugin 卸载（如已全局安装）
claude plugin uninstall wk-im-dev@yuxilong-agents
claude plugin marketplace remove yuxilong-agents
```

卸载会移除 `~/.wk-im-dev/`、`~/.codex/agents/wk-im-dev.toml`、`~/.codex/config.toml` 中的 profile 区块、shell rc 中的 PATH 行，以及（如指定 `--target`）目标仓库 `AGENTS.md` 中的 wk-im-dev 区块。`docs/agent-knowledge/` 知识库目录会被保留，如不再需要可手动删除。

**首次知识库生成包含什么？**

采用混合方案：脚本（`wk-im-kb-scan.sh`）在 generated block 中生成静态结构——目录树、public header 方法签名、protocol conformance 列表，以及按业务关键词（未读数、会话、输入框、会话列表）搜索出的相关文件引用。**调用者映射已下沉到 CodeGraph**（`codegraph_callers` / `codegraph_impact`），grep 跟不上的 Swift↔ObjC bridging / @selector / 动态分发都能覆盖。setup 完成后，agent 会检测是否首次初始化，如果是，则自动委派 `wk-im-knowledge-maintainer` 深度填充 `topics/` 下的业务知识页（填充 `common-flows.md` 中的占位符、创建 `unread-count.md`/`message-status.md` 等），使后续提问可直接从知识库获取答案而无需重新探索代码库。

**CodeGraph 与 Knowledge Base 怎么分工？**

- **调用关系 / 影响半径 / 流程追踪** → CodeGraph（AST 索引，实时更新）
- **组件入口、业务 topic、curated notes、架构决策** → Knowledge Base（人工 + 脚本维护）
- **Public API 签名** → 两者互补：wiki 给清单，codegraph 给精准 caller
- **Cross-pod 契约校验** → `contracts.md` + `codegraph_impact` 双源

CodeGraph 未安装时 agent 自动回退到 wiki + grep，功能不丢失。详见 [docs/codegraph-integration.md](docs/codegraph-integration.md)。
...
- **v3.3.0** — 集成 CodeGraph（AST 索引），subagent 命名统一为 `wk-im-*` 前缀；硬约束拆分为 core + extended 两层，subagent 上下文节省约 5k token；kb-refresh 引入 5 分钟去重窗口；verifier 按 diff 类型触发验证子集
- **v3.2.0** — `plugin.json` name 统一为 `wk-im-dev`，Codex TOML 精简去重，guard 隐私检查优化，core 添加 YAML frontmatter
- **v3.1.0** — 知识库 LLM Wiki 系统（`docs/agent-knowledge/`），`wk-im-kb-scan.sh`/`wk-im-kb-check.sh` 自动维护
- **v3.0.0** — 重大重构：聚焦 BTIMService/BTIMModule iOS 组件，Claude Code plugin 适配，6 子 Agent 分工
- **v0.3.0** — 重命名为 `wk-im-dev`，Codex + Claude Code 双平台支持
- **v0.2.0** — 添加多 Agent 协作（Claude Code）
- **v0.1.0** — 初始版本（仅 Codex）
