...
> ⚠️ **请直接装 v1.1.1**：v1.1.0 发布后端到端发现 `.claude-plugin/plugin.json` 冗余声明 `hooks` 字段，导致 Claude plugin 加载报 `Duplicate hooks file detected`、hooks 全失效。v1.1.1 已修复，v1.1.0 tag 保留但不推荐使用。详见 [CHANGELOG v1.1.1](CHANGELOG.md#v111--2026-06-15-hotfix)。
...
```bash
# Codex
curl -fsSL https://raw.githubusercontent.com/YuXilong-Labs/Agents/v1.1.1/wk-im-dev/scripts/bootstrap.sh \
  | bash -s -- --target . --ref v1.1.1
# Claude Code
...
| 安装指定版本（推荐） | `curl ... bootstrap.sh \| bash -s -- --target <repo> --ref v1.1.1` |
