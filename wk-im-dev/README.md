...
# BTIMModule 同理：
# ~/.wk-im-dev/bin/wk-im-init.sh --root /path/to/BTIMModule
```

#### 场景 B：在 HostApp 中通过 CocoaPods 本地依赖同时开发两个组件

适用于 Podfile 已配置本地 `:path =>`，需要跨仓库联调。
...
# 成功后生成：
#   /path/to/HostApp/.wk-im-workspace.json  （项目级，记录两个组件路径）
#   ~/.wk-im-dev/workspace.json             （全局，guard/verify 的跨仓库 fallback）
```
...
**Codex**：在仓库目录运行 `codex`
**Claude Code**：在仓库目录运行 `claude`（插件已全局生效）
...
| Claude Code 安装 | `/plugin install wk@YuXilong-Labs` 或 `claude --plugin-dir <wk-im-dev>` |
| HostApp 同时改两个组件 | `bootstrap.sh --target <HostApp>` + `wk-im-init.sh --root <HostApp>` |
| 路径自动识别失败 | `wk-im-init.sh --service <p> --module <p> --host-app <p>` |
...
插件安装后在任意仓库运行 `claude` 即可使用，无需写 `CLAUDE.md`。
