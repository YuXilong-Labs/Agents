...
#### Claude Code — 显式加载本地 plugin

```bash
claude --plugin-dir /path/to/Agents/wk-im-dev
```

plugin 只在显式指定 `--plugin-dir` 时生效，默认 Claude Code 会话**不会**激活本 agent。
...
**Claude Code**：`claude --plugin-dir /path/to/Agents/wk-im-dev`（显式加载，不影响其他会话）
...
| Claude Code 加载 | `claude --plugin-dir /path/to/Agents/wk-im-dev`（显式，不全局生效） |
...
`--plugin-dir` 只对当前会话生效，不影响其他 Claude Code 会话，也不写入任何全局配置。这是推荐的激活方式，确保本 agent 只在需要 IM 开发时生效。
