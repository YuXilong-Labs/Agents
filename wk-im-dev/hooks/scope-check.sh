...
for lib in "${CLAUDE_PLUGIN_ROOT:-}/bin/wk-im-components.sh" \
           "$HOME/.wk-im-dev/bin/wk-im-components.sh" \
           "$(cd "$(dirname "$0")/../bin" 2>/dev/null && pwd)/wk-im-components.sh"; do
...
if echo "$TARGET" | grep -qE "$PATTERN"; then
  ...
  echo "wk-im-dev 默认只允许修改已配置的组件目录。" >&2
