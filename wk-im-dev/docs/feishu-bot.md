# wk-im-dev 飞书 Bot 部署指南

> 适用于：把 wk-im-dev 作为飞书机器人接入团队群，群里 @ 机器人即可触发 IM 组件相关问答 / 代码定位 / 简单 review。
> 这是**示例集成**，不是 wk-im-dev 的核心能力 — 维护成本归调用方。

---

## 适用场景

- 团队群里随时问"消息发送流程怎么走"，机器人答出关键文件 + 调用链
- PR 描述里贴 diff 摘要，机器人按 IM 边界规则做轻量 review
- 新人入职在群里直接问，不必每次自己装 wk-im-dev

不适用：

- 实际改代码（需要 plugin 在本机运行，bot 没有 push 权限）
- 长链路调试（推荐还是本地 `wk-im-dev` 会话）
...
机器人本质是用 Claude Agent SDK 启动一个 `ClaudeSDKClient`，把 wk-im-dev plugin 当 local plugin 加载，再用 lark-oapi 监听群消息。
...
export PLUGIN_DIR="/path/to/Agents/wk-im-dev"
export PROJECT_DIR="/path/to/BTIMService"   # 机器人默认工作目录
...
| Docker | 把 wk-im-dev 仓库 + `PROJECT_DIR` 挂进容器；注意 plugin 路径要对 |
| K8s + Helm | 生产团队推荐；用 Secret 管 `FEISHU_APP_*`，PVC 挂 BTIMService/BTIMModule 源码 |
...
这些都属于团队各自的工程化范围，wk-im-dev 不提供 opinionated 实现。
