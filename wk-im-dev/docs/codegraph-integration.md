...
`wk-im-dev` 集成 [CodeGraph](https://github.com/colbymchenry/codegraph) — 基于 tree-sitter AST 的本地代码知识图谱（SQLite），用于加速 agent 的结构化查询并节省 token。
...
CodeGraph 在 Wikipedia-iOS 等大型 ObjC+Swift 工程上经过验证，对 BTIMService（ObjC + 部分 Swift） 的解析与 caller/callee 识别准确率可用于 agent 决策。

## 安装与初始化

`wk-im-init.sh` 已经自动集成 codegraph 检测与初始化：
...
```bash
# 检测
~/.wk-im-dev/bin/wk-im-codegraph.sh detect
...
# 查看索引状态
~/.wk-im-dev/bin/wk-im-codegraph.sh status --root /path/to/BTIMService
```

非交互模式（CI、脚本调用）：

```bash
wk-im-codegraph.sh install --yes --quiet
wk-im-codegraph.sh init --root <repo> --yes --quiet
```

## Agent 调用顺序

`wk-im-explorer` 与 `wk-im-debugger` 的优先级：
...
`wk-im-verifier` 会在 public header 变更时调 `codegraph_impact` 评估影响半径。
...
Wiki 不再生成 "Callers of …" 段（已在 `wk-im-kb-scan.sh` 中删除）。所有调用关系问题由 codegraph 回答。
...
`wk-im-dev` 不会强依赖 codegraph，卸载后 agent 自动回退到 grep + wiki。
