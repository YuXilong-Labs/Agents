...
```markdown
---
component: BTIMService
kind: topic
generated_by: wk-im-kb-scan.sh
last_verified_commit: abc1234
---

# Page Title

<!-- WK-IM-GENERATED:START -->
...
The generated block belongs to `wk-im-kb-scan.sh`.
Curated notes and source refs belong to humans or the `im-knowledge-maintainer` agent.
...
- Run `wk-im-kb-scan.sh --root <repo>` before broad code search when the wiki is missing or stale.
...
- Run `wk-im-kb-check.sh --root <repo>` before reporting completion.
