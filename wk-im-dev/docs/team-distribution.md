...
WK_IM_DEV_REF="v1.0.1" \
  curl -fsSL https://gitlab.intra/team/Agents/-/raw/v1.0.1/wk-im-dev/scripts/bootstrap.sh \
  | bash -s -- --target .
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/YuXilong-Labs/Agents/v1.0.1/wk-im-dev/scripts/bootstrap.sh \
  | bash -s -- --target /path/to/BTIMService --ref v1.0.1
```
...
# 4. 运行
bash /tmp/wk-im-dev-bootstrap.sh --target . --ref v1.0.1
```
...
2. 安装 wk-im-dev：
   ```
   WK_IM_DEV_REF=v1.0.1 curl -fsSL .../bootstrap.sh | bash -s -- --target .
   ```
3. 验证：
   ```
