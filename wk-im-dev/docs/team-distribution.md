...
WK_IM_DEV_REF="v1.0.0" \
  curl -fsSL https://gitlab.intra/team/Agents/-/raw/v1.0.0/wk-im-dev/scripts/bootstrap.sh \
  | bash -s -- --target .
```
...
发布给团队的安装命令里**始终带 `--ref v1.0.0`**，避免某次 `main` 上的未稳定 commit 让全员安装挂掉。
...
docker run --rm -it -v $PWD:/work ubuntu:22.04 bash -c '
  ...
  curl -fsSL https://raw.githubusercontent.com/YuXilong-Labs/Agents/v1.0.0/wk-im-dev/scripts/bootstrap.sh \
    | bash -s -- --target /work --ref v1.0.0 --no-shell-rc --skip-init
  ls ~/.wk-im-dev/bin
'
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/YuXilong-Labs/Agents/v1.0.0/wk-im-dev/scripts/bootstrap.sh \
  | bash -s -- --target /path/to/BTIMService --ref v1.0.0
```
...
curl -fsSL .../bootstrap.sh | bash -s -- --target . --ref v1.0.0
```
...
| 症状 | 原因 / 修复 |
|---|---|
| `git clone --branch v1.0.0` 失败 | tag 没推到默认 remote → 发布者 `git push origin v1.0.0` |
...
# 4. 运行
bash /tmp/wk-im-dev-bootstrap.sh --target . --ref v1.0.0
```
...
2. 团队基础镜像 / Mac 准备脚本里集成：
   ```bash
   git clone --depth 1 --branch v1.0.0 https://gitlab.intra/team/Agents.git /opt/Agents
...
2. 安装 wk-im-dev：
   ```
   WK_IM_DEV_REF=v1.0.0 curl -fsSL .../bootstrap.sh | bash -s -- --target .
   ```
3. 验证：
   ```
   wk-im-dev --version    # 应输出 1.0.0
