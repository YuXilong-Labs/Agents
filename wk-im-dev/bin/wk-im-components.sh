#!/bin/bash
# wk-im-components.sh — 组件清单(components.conf)的定位与解析库。
...
#   1. $WK_IM_DEV_COMPONENTS 环境变量
#   2. ${CLAUDE_PLUGIN_ROOT}/components.conf （plugin-native）
#   3. ~/.wk-im-dev/components.conf           （installer 复制）
#   4. 相对本库的源码位置                      （开发场景）
wk_components_conf() {
  local c
  for c in "${WK_IM_DEV_COMPONENTS:-}" \
           "${CLAUDE_PLUGIN_ROOT:-}/components.conf" \
           "$HOME/.wk-im-dev/components.conf" \
