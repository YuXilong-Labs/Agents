...
iOS 视频编辑组件开发 Agent，用于 `VideoEditCore` 和 `VideoEditUI` 的功能开发、Bug 修复、代码审查、架构查询和组件知识库维护。

让 Codex / Claude Code 在改视频编辑代码前先快速定位入口、遵守跨 Pod 边界，并在源码变化后同步维护 `docs/agent-knowledge/`。
...
>
> 本 agent 由仓库根 `tools/create-wk-agent.sh` 从 `manifests/video-edit.json` 生成（模板=wk-im-dev）。
...
> 本地 pod 改源文件直接 build，无需 `pod install`；跨仓库提交顺序：先 commit VideoEditCore（含 public header + contracts），再 commit VideoEditUI。
...
| 规则 | 说明 |
|---|---|
| `VideoEditCore` 不得 import `VideoEditUI` | 依赖方向单向（UI → Core → SDK） |
| `VideoEditUI` 不得 import `VideoEngineSDK` | 第三方视频引擎只在 Core adapter 层访问 |
| 默认只修改 `VideoEditCore/` 与 `VideoEditUI/` | 防止误伤宿主 App 或依赖副本 |
| 不在日志暴露 `components.conf` 的 privacy 字段（sourceURL/exportPath/licenseKey 等）与 PII | 隐私保护 |
| Public API 变更必须更新 knowledge contracts | 契约治理 |

> `VideoEditUI` 可正常使用系统 `AVFoundation`（如 `AVPlayerLayer` 预览）；受约束的是**第三方视频引擎 SDK**，必须经 `VideoEditCore` adapter。
