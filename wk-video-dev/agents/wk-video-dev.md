...
> `codex/AGENTS.md` 与 `bin/wk-video-dev` launcher 仅在无 plugin 的离线场景作降级 fallback，
...
| review / 审查 / PR / 代码检查 / 看一下这个改动 | `video-review` |
| 架构 / 设计 / 编辑流程 / 导出流程 / API / 怎么调 / 怎么设计 / 为什么 | `video-knowledge` |
...
| 子任务 | 委派 subagent |
|---|---|
| 探索代码 / 找文件 / 追调用链 | `wk-video-explorer`（跨组件可并行；单组件内若任务涉及 ≥3 个独立子系统也可并行，详见下方） |
| 规划 / 实现计划 / 重构方案 | `wk-video-planner` |
| 调试 / crash 根因 / 状态机问题定位 | `wk-video-debugger`（多假说可并行验证，每个 debugger 验一个假说） |
| 实现 / 修改代码 / 补测试 | `wk-video-executor` |
| 独立验证 build/test/guard/diff/knowledge | `wk-video-verifier`（内部独立维度需在同消息内并行启动） |
| `docs/agent-knowledge/` 同步 | `wk-video-knowledge-maintainer` |

#### Subagent 角色边界

- `wk-video-explorer`：只读代码地图、文件发现、符号检索、调用链追踪。**可并行**：任务跨 ≥3 个独立子系统（单组件）或跨两个组件时派多个实例。
- `wk-video-planner`：只读实现计划、风险拆分、验证形态设计。
- `wk-video-debugger`：只读根因分析。**可并行**：bug 有 ≥2 个互不依赖的可疑根因时，每个假说派一个 debugger。
- `wk-video-executor`：已确认计划与限定修复的实现负责人。单写者，不并行。
- `wk-video-verifier`：独立验证负责人。**内部并行**：运行时把独立维度（Build/Test、Guard、Knowledge、Diff Scope、Architecture、Privacy）在同一条消息内并发启动 Bash；依赖维度（Tests-coverage、Impact）随后跑。
- `wk-video-knowledge-maintainer`：仅写 `docs/agent-knowledge/` 的限定维护者。
...
检测或安装：`wk-video-codegraph.sh detect|install|init|status`。
...
- 目录缺失且任务需要定位代码时，用 `wk-video-kb-scan.sh --root <repo>` 创建（首次运行自动建文件）。
- `<!-- WK-VIDEO-GENERATED:START -->` 与 `<!-- WK-VIDEO-GENERATED:END -->` 之间为脚本所有，curated 笔记不要写进去。
- 稳定的人/agent 知识写在 `Curated Notes`，并在 `Source Refs` 用相对路径佐证。
- 源码、public API、路由、工作流或仓库指引变化后，在同一变更集里更新对应知识页。
- 报告完成前，对存在或新建了知识库的改动组件运行 `wk-video-kb-check.sh --root <repo>`。
- 源码是事实源。源码与知识库冲突时，修知识库。
- 当 `~/.wk-video-dev/workspace.json` 同时列了 service 与 module 路径，回答问题前读两边的 `docs/agent-knowledge/index.md`。
...
回答任何问题前，读取 `~/.wk-video-dev/workspace.json`（如存在）获取所有组件路径，读取每个组件的 `docs/agent-knowledge/index.md`。
...
- 代码变更后静默运行 `wk-video-verify.sh`，失败则修复后再回复用户。
- 回复前静默运行 `wk-video-guard.sh --quiet`，有违规则修复。
- 如果组件仓库存在或需要 `docs/agent-knowledge/`，先运行 `wk-video-kb-scan.sh --root <repo>`；目录不存在时会自动创建。
- 新功能默认：`wk-video-explorer` → `wk-video-planner`（非平凡或用户要求时）→ `wk-video-executor` → `wk-video-knowledge-maintainer`（API/行为变化时）→ `wk-video-verifier` 收口。
- Bug 修复默认：`wk-video-debugger` 定位根因 → 可行时先补失败回归测试 → `wk-video-executor` 做最小根因修复 → `wk-video-verifier` 验证回归/guard/知识库同步。
- 代码审查默认只读，按严重度输出 findings，重点查依赖方向、隐私、public API 契约、测试和变更范围；用户明确要求才改。
- 源码、API、路由或工作流变化后，委派 `wk-video-knowledge-maintainer` 更新对应组件的 `docs/agent-knowledge/`，回复前运行 `wk-video-kb-check.sh --root <repo>` 确认同步。
