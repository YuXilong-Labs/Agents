# wk-video-dev 架构与运行原理

> 本文档面向「想理解或重画 wk-video-dev 架构图」的读者。所有 Mermaid 代码块可直接渲染。

> ⚠️ **2026-06-15 起的变更（部分图待刷新）**：Codex 路径已转为 plugin-native——
> 行为契约的**唯一事实源**是 `agents/wk-video-dev.md`（不再有 `core/wk-video-dev-core.md`）；
> Codex 激活走 plugin `agents/` + `SessionStart` hook（`hooks/session-init.sh`）+ `/wk-video-dev` 命令，
> 不再安装 `~/.codex/agents/wk-video-dev.toml` 与 `~/.codex/wk-video-dev.config.toml`（profile）。
> launcher 离线 fallback 现注入 `~/.wk-video-dev/wk-video-dev-agent.md`。
> 下方提到 `core spec 复制`、`wk-video-dev.toml → ~/.codex/agents/`、`profile.toml 合并 config.toml` 的图块属旧流程，待整体重绘。
...
graph TB
  ...
  subgraph Launcher["统一入口层"]
    L["wk-video-dev launcher\n(bin/wk-video-dev)"]
    D["wk-video-dev doctor"]
  end
  ...
  subgraph Persona["人格 / 路由层"]
    MA["主 Agent: wk-video-dev"]
    CORE["core/wk-video-dev-core.md\n(共享事实源)"]
    AGENTS["codex/AGENTS.md\n(项目入口)"]
  end

  subgraph Subagents["Subagent 分工层"]
    EXP[wk-video-explorer]
    PLN[wk-video-planner]
    DBG[wk-video-debugger]
    EXE[wk-video-executor]
    VER[wk-video-verifier]
    KMA[wk-video-knowledge-maintainer]
  end
  ...
  subgraph Tools["工具与外部依赖"]
    ...
    WS["~/.wk-video-dev/workspace.json\n(组件路径配置)"]
    CG["CodeGraph\n(AST 索引, 可选)"]
...
flowchart TD
  ...
  F --> G["复制 wk-video-dev.toml → ~/.codex/agents/"]
  G --> H["复制 core spec → ~/.wk-video-dev/wk-video-dev-core.md"]
  H --> I["复制 bin/*.sh + launcher → ~/.wk-video-dev/bin/"]
  I --> J[追加 shell rc PATH]
  J --> K["写 [profiles.wk-video-dev] 到 ~/.codex/config.toml"]

  K --> L{looks_like_im_repo?}
  L -- 是 --> M["自动调用 wk-video-init.sh --root <target> --quiet"]
  L -- 否 --> N[只装不 init,打印提示]

  M --> M1[wk-video-detect-env.sh 解析仓库类型]
  M1 --> M2["写 ~/.wk-video-dev/workspace.json\n(service/module/hostApps)"]
  M2 --> M3["wk-video-kb-bootstrap.sh\n创建 docs/agent-knowledge/ 骨架"]
  M3 --> M4["wk-video-kb-scan.sh\n刷新 generated block"]
  M4 --> M5["wk-video-kb-check.sh\n校验链接/完整性"]
  M5 --> M6{CodeGraph 已装?}
  M6 -- 是 --> M7["wk-video-codegraph.sh init\n每个 scan_root 建 .codegraph/"]
...
flowchart LR
  ...
  RC -->|exec| CC["claude --agent wk-video-dev $@"]
  RX -->|exec| CX["codex -c developer_instructions=<core> \\\n  --profile wk-video-dev (或 -c model/effort fallback) $@"]
...
- `~/.claude/settings.json` 含 `"wk-video-dev"` 字符串
- 或当前目录是 plugin 源码（`./.claude-plugin/plugin.json` 含 `"wk-video-dev"`，用于 `--plugin-dir` 调试模式）
...
flowchart TD
  ...
  I1 -- review/PR --> F3[video-review skill 只读]
  I1 -- 架构/设计/API/导出流程 --> F4[video-knowledge skill]
  I1 -- 探索/找文件/调用链 --> F5[wk-video-explorer subagent]
  I1 -- plan/重构 --> F6[wk-video-planner subagent]
  I1 -- 实现/补测试 --> F7[wk-video-executor subagent]
  I1 -- 验证/build/test --> F8[wk-video-verifier subagent]
  I1 -- 知识库/agent-knowledge --> F9[wk-video-knowledge-maintainer]
...
sequenceDiagram
  autonumber
  actor U as User
  participant MA as 主Agent (wk-video-dev)
  participant E as wk-video-explorer
  participant P as wk-video-planner
  participant X as wk-video-executor
  participant K as wk-video-knowledge-maintainer
  ...
  MA->>V: 运行 verifier
  V->>V: wk-video-verify.sh (build/test/guard)
  V->>V: wk-video-kb-check.sh
  V->>V: diff 范围核对
  V-->>MA: 全部通过

  MA-->>HK: 会话即将结束 → Stop hook
  HK->>HK: wk-video-guard.sh --quiet
  HK-->>MA: 通过
...
**对非视频项目的安全性**：三个 hook 都内置"快速 no-op"路径（找不到 podspec、env=unknown 等），平均开销 < 5ms，不污染其他项目。
...
- `<!-- WK-VIDEO-GENERATED:START/END -->` 之间是脚本所有，agent 不写
...
约束的事实源：`skills/video-knowledge/constraints.md`（拆分为 `constraints-core.md` 给 subagent / `constraints-extended.md` 给主 agent，节省上下文）。
...
─────────────────────────          ────────────────────────────────
wk-video-dev/                         ~/.wk-video-dev/
├── .claude-plugin/                ├── wk-video-dev-core.md         (从 core/ 复制)
│   └── plugin.json                ├── workspace.json            (由 init 写)
├── agents/                        └── bin/
│   ├── wk-video-dev.md                   ├── wk-video-dev             (launcher)
│   ├── wk-video-explorer.md              ├── wk-video-init.sh
│   ├── wk-video-planner.md               ├── wk-video-detect-env.sh
│   ├── wk-video-debugger.md              ├── wk-video-kb-scan.sh
│   ├── wk-video-executor.md              ├── wk-video-kb-check.sh
│   ├── wk-video-verifier.md              ├── wk-video-kb-bootstrap.sh
│   └── wk-video-knowledge-maintainer.md  ├── wk-video-guard.sh
├── bin/                               ├── wk-video-verify.sh
│   └── (同上,源)                       └── wk-video-codegraph.sh
├── codex/
│   ├── AGENTS.md  ──→ 合并到目标仓库 <target>/AGENTS.md
│   ├── wk-video-dev.toml ──→ ~/.codex/agents/wk-video-dev.toml
│   └── profile.toml ──→ 合并到 ~/.codex/config.toml
│                                  ~/.codex/
├── core/                          ├── agents/wk-video-dev.toml
│   └── wk-video-dev-core.md          └── config.toml ([profiles.wk-video-dev] 块)
├── docs/
│   ├── architecture.md (本文)     <target repo>/
│   ├── advanced-install.md        ├── AGENTS.md  (含 WK-VIDEO-DEV marker 块)
│   ├── agent-knowledge.md         ├── docs/agent-knowledge/      (init 写)
│   └── codegraph-integration.md   └── .codegraph/                (可选)
├── hooks/
│   ├── hooks.json
│   ├── scope-check.sh             ~/.zshrc 或 ~/.bashrc
│   └── kb-refresh.sh              # wk-video-dev
├── scripts/                       export PATH="$HOME/.wk-video-dev/bin:$PATH"
│   ├── bootstrap.sh
│   ├── install.sh
│   ├── uninstall.sh
│   └── verify.sh
...
stateDiagram-v2
  ...
  已安装 --> 已初始化: install 自动触发\n或手动 wk-video-init.sh
  已初始化 --> 会话就绪: wk-video-dev 启动
...
| 统一 launcher | `wk-video-dev` 自动派发 | 用户只记一个命令 |
...
| Workspace 配置 | `~/.wk-video-dev/workspace.json` 全局单文件 | 任意仓库下都能读到组件路径，跨仓联调 |
...
| 安装幂等 | marker 包裹 (`WK-VIDEO-DEV:START/END`) | 多次安装不复写，可干净卸载 |
| 自检 | 首次激活检 workspace.json + `wk-video-dev doctor` | 出错时一行可见 |
...
需要更精简的 high-level 单张图，用第 1 节；需要面试讲解流程，用第 5 节 + 第 12 节表格；需要排错，看第 6 节 + `wk-video-dev doctor`。
