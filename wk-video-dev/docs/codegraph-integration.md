...
CodeGraph 在 Wikipedia-iOS 等大型 ObjC+Swift 工程上经过验证，对 BTVideoRecorderKit（ObjC + 部分 Swift） 的解析与 caller/callee 识别准确率可用于 agent 决策。
...
```bash
~/.wk-video-dev/bin/wk-video-init.sh --root /path/to/BTVideoRecorderKit
# 会自动：
#   1. 检测 codegraph 是否安装
...
# 给指定 root 建索引
~/.wk-video-dev/bin/wk-video-codegraph.sh init --root /path/to/BTVideoRecorderKit

# 查看索引状态
~/.wk-video-dev/bin/wk-video-codegraph.sh status --root /path/to/BTVideoRecorderKit
```
