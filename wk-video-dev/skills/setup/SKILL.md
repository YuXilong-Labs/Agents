...
2. 如果当前目录是 `VideoEditCore` 或 `VideoEditUI`，初始化脚本会直接创建/刷新该仓库的 `docs/agent-knowledge/`。
3. 如果当前目录是 HostApp，初始化脚本会从 Podfile 的本地 `:path =>` 解析 `VideoEditCore` 和 `VideoEditUI`，写入 `~/.wk-video-dev/workspace.json`，并刷新两个组件仓库的知识库。
4. 支持多个 HostApp，每个用单独的 `--host-app` 指定：

```bash
${CLAUDE_PLUGIN_ROOT}/bin/wk-video-init.sh \
  --service <VideoEditCore> \
  --module  <VideoEditUI>  \
  --host-app <HostApp1>   \
  --host-app <HostApp2>
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/wk-video-init.sh \
  --root <当前工作区> --service <VideoEditCore> --module <VideoEditUI> --host-app <HostApp>
```
