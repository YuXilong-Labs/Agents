...
write_generated_page() {
  ...
  if [ -f "$path" ] \
  ...
  else
    {
      ...
      echo "kind: $kind"
      echo "generated_by: wk-video-kb-scan.sh"
...

{
  ...
  echo "## Maintenance"
  echo ""
  echo "- Run \`wk-video-kb-scan.sh --root \"$ROOT\"\` after meaningful source changes."
  echo "- Run \`wk-video-kb-check.sh --root \"$ROOT\"\` before reporting completion."
