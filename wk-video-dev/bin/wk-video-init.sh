#!/bin/bash
# wk-video-init.sh
# Initialize wk-video-dev workspace detection and component knowledge bases.
# Workspace config is written only to ~/.wk-video-dev/workspace.json (global).
#
...
# shellcheck source=wk-video-components.sh
. "$SCRIPT_DIR/wk-video-components.sh"

usage() {
  cat <<'USAGE'
Usage: wk-video-init.sh [--root <repo>] [--component <Name>=<path>]
                     [--service <path>] [--module <path>]
                     [--host-app <path>] [--host-app <path2> ...]
                     [--with-codegraph] [--quiet]

Initializes wk-video-dev for a component repo or host app workspace. Component names
are read from components.conf. Detects each component's path, writes workspace
config to ~/.wk-video-dev/workspace.json, then scans/checks docs/agent-knowledge/.
...
~/.wk-video-dev/workspace.json if nothing matches.
USAGE
}
...
read_workspace_fallback() {
  local cfg="$HOME/.wk-video-dev/workspace.json"
  [ -f "$cfg" ] || return 1
...
if [ -z "$ROOT" ]; then
  ...
  if AUTO="$(walk_up_for_im_root "$(pwd)")"; then
    [ "$QUIET" -eq 1 ] || echo "Auto-detected video repo root: $AUTO"; ROOT="$AUTO"
  elif AUTO="$(read_workspace_fallback)"; then
    [ "$QUIET" -eq 1 ] || echo "Using workspace fallback: $AUTO (from ~/.wk-video-dev/workspace.json)"; ROOT="$AUTO"
  else
    ROOT="$(pwd)"
  fi
fi
...
[ -x "$SCRIPT_DIR/wk-video-detect-env.sh" ] || fail "Missing executable: $SCRIPT_DIR/wk-video-detect-env.sh"
[ -x "$SCRIPT_DIR/wk-video-kb-scan.sh" ]    || fail "Missing executable: $SCRIPT_DIR/wk-video-kb-scan.sh"
[ -x "$SCRIPT_DIR/wk-video-kb-check.sh" ]   || fail "Missing executable: $SCRIPT_DIR/wk-video-kb-check.sh"

# ----- seed component map: detected → existing workspace → explicit overrides --
DETECTED_JSON="$("$SCRIPT_DIR/wk-video-detect-env.sh" "$ROOT")"
ENV_NAME="$(printf '%s' "$DETECTED_JSON" | grep -o '"env":"[^"]*"' | cut -d'"' -f4)"
GLOBAL_CONFIG="$HOME/.wk-video-dev/workspace.json"
...
for scan_root in "${SCAN_ROOTS[@]}"; do
  ...
  if [ "$QUIET" -eq 1 ]; then
    "$SCRIPT_DIR/wk-video-kb-scan.sh" --root "$scan_root" --quiet
  else
    "$SCRIPT_DIR/wk-video-kb-scan.sh" --root "$scan_root"
  fi
  "$SCRIPT_DIR/wk-video-kb-check.sh" --root "$scan_root"
done

# CodeGraph: only auto-install when --with-codegraph is passed.
if [ -x "$SCRIPT_DIR/wk-video-codegraph.sh" ]; then
  CG_FLAGS="--yes"
  [ "$QUIET" -eq 1 ] && CG_FLAGS="--quiet --yes"
  if "$SCRIPT_DIR/wk-video-codegraph.sh" detect --quiet >/dev/null 2>&1; then
    for scan_root in "${SCAN_ROOTS[@]}"; do
      if [ ! -d "$scan_root/.codegraph" ]; then
        [ "$QUIET" -eq 1 ] || echo "Initializing codegraph index: $scan_root"
        "$SCRIPT_DIR/wk-video-codegraph.sh" init --root "$scan_root" $CG_FLAGS || true
      fi
    done
  elif [ "$WITH_CODEGRAPH" -eq 1 ]; then
    [ "$QUIET" -eq 1 ] || echo ""
    [ "$QUIET" -eq 1 ] || echo "Installing CodeGraph (--with-codegraph) ..."
    if "$SCRIPT_DIR/wk-video-codegraph.sh" install $CG_FLAGS; then
      for scan_root in "${SCAN_ROOTS[@]}"; do
        [ "$QUIET" -eq 1 ] || echo "Initializing codegraph index: $scan_root"
        "$SCRIPT_DIR/wk-video-codegraph.sh" init --root "$scan_root" $CG_FLAGS || true
      done
    ...
    [ "$QUIET" -eq 1 ] || echo "  Enable later: ~/.wk-video-dev/bin/wk-video-codegraph.sh install && \\"
    [ "$QUIET" -eq 1 ] || echo "                ~/.wk-video-dev/bin/wk-video-codegraph.sh init --root <repo>"
  fi
fi

if [ "$QUIET" -ne 1 ]; then
  echo ""
  echo "wk-video-dev initialization finished."
  echo "Codex:       cd \"$ROOT\" && codex"
  echo "Claude Code: claude --plugin-dir \"$(cd "$SCRIPT_DIR/.." && pwd)\""
  echo ""
  echo "Setup via agent:"
  echo "  Claude Code: /wk-video-dev:setup"
  echo "  Codex:       \$wk-video-dev:setup"
fi
