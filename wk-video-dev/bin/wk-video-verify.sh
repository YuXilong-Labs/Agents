...
# Usage: wk-video-verify.sh [--build-only]

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
GLOBAL_CONFIG="$HOME/.wk-video-dev/workspace.json"

ENV_JSON=$("$SCRIPT_DIR/wk-video-detect-env.sh" 2>/dev/null || echo '{"env":"unknown"}')
...
if [ -z "$HOST_APP" ] || [ ! -d "$HOST_APP" ]; then
  ...
  echo "   Run: /wk-video-dev:setup  (Claude Code)"
  echo "   Or:  \$wk-video-dev:setup  (Codex)"
  exit 1
fi
