...
ENV_JSON=$("$SCRIPT_DIR/wk-video-detect-env.sh" 2>/dev/null || echo '{"env":"unknown","components":{}}')
...
# Fast exit for non-video repos — safe as a globally-installed plugin hook.
if [ "$ENV" = "unknown" ] && [ ! -f "$GLOBAL_CONFIG" ]; then
  [ "$QUIET" != "--quiet" ] && echo "✅ Guard skipped (not a video repo)."
  exit 0
fi
